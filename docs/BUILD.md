...
4. Under **Artifacts**, download `endstone_oniprofiler.so` for Linux or `endstone_oniprofiler.dll` for Windows. The `OniProfiler-linux-x86_64` and `OniProfiler-windows-x86_64` artifacts also include release ZIPs and matching source archives.
...
The archive supplied through this chat is source, not a completed native build. The full workflow has not run here. Dependency access, native compilation and BDS loader/runtime verification are explicit release gates.
...
The two native jobs install Clang/libc++ 20 on Ubuntu 24.04 or LLVM/clang-cl 20.1.8 plus the x64 Windows SDK on Windows 2025. The Windows installer has a pinned official SHA-256. The Linux apt signing key fingerprint is verified. Conan 2.29.1 resolves the declared versions, records recipe revisions in a per-run lock, and builds missing dependencies.
...
A pushed tag exactly matching `VERSION`, initially `v1.0.0-rc.1`, assembles a **draft prerelease only after all jobs pass**. Review the staging checklist before publishing it. Build success alone is not a claim of BDS compatibility or performance overhead.
