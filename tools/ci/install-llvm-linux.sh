...
set -euo pipefail
. /etc/os-release
[[ "${VERSION_CODENAME}" == jammy ]] || { echo "This setup script requires Ubuntu 22.04 (jammy) for the supported GLIBC baseline." >&2; exit 1; }
sudo apt-get update
...
printf '%s\n' 'deb [signed-by=/usr/share/keyrings/oniprofiler-llvm.gpg] https://apt.llvm.org/jammy/ llvm-toolchain-jammy-20 main' | sudo tee /etc/apt/sources.list.d/oniprofiler-llvm.list >/dev/null
sudo apt-get update
