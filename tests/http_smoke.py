...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='oni-http-') as folder:
        ...
        payload={'schema_version':1,'product':'OniProfiler powered by spark','version':'1.0.0-rc.1','kind':'health','instance_id':'b'*32,'generated_ms':now_ms(),
        ...
        with (root/'server.log').open('w') as log:
            ...
            try:
                with httpx.Client(base_url=origin,timeout=5,trust_env=False) as client:
                    ...
                    assert response.json()['version']=='1.0.0-rc.1';checks+=1
